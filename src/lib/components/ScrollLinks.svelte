...
{#each data.header[0].data.links as link}
    <a href={link.url} on:click|preventDefault={() => scrollToAnchor(link.url.replace('#', ''))}>
        {link.text} 
    </a>
{/each}