<script lang="ts">
	...
	export let subheadline: string | null = null;
	...
	// Use provided items or global testimonials
	$: testimonials = items || globalTestimonials;
	$: displayHeadline = headline || globalHeadline;
	$: displaySubheadline = subheadline || globalSubheadline;
	$: showHeadlines = (displayHeadline || displaySubheadline) && !items;
	$: hasProvidedItems = items && items.length > 0;
	$: shouldShow = testimonials.length > 0 && (hasProvidedItems || (!loading && globalTestimonials.length > 0));
	...
	// Map global testimonial to component format (image -> portrait)
	...
	onMount(async () => {
		// Only fetch if items not provided
	...
	<section class="pb-14">
		...
		<Slider>
			{#each testimonials as item, index}
				<div class="item relative rounded-lg overflow-hidden flex-shrink-0 bg-[var(--secondary-color)] aspect-[3/4] lg:aspect-[16/7]" style={calculateStyles(index, testimonials.length)}>
